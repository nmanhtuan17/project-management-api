...
export class AuthController {
  ...
  async login(@Body() loginDto: LoginDto) {
    ...
    if (!user)
      throw new UnauthorizedException(Messages.auth.wrongEmailPassword);
    if (
      !this.config.get('auth.ignoreMailVerification') &&
      !user.emailVerified
    ) {
      throw new HttpException(
        Messages.auth.emailNotVerified,
  ...
  @Post('verify')
  public async verifyAccount(@Body() info: VerifyUserDto) {
    const { email, code } = info;
    const user = await this.db.user.findOne({ email });
    if (!user)
      throw new ApiError(Messages.common.invalidEmail, HttpStatus.BAD_REQUEST);
  ...
  async resetPassword(@Body() payload: ResetPasswordDto) {
    ...
    return {
      message: Messages.auth.passwordWasSent,
      status: HttpStatus.OK,
    };
  }
